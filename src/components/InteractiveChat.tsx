  ...
  return (
    <section id="contato" className="py-12 max-w-6xl mx-auto px-4">
      <motion.div 
        ...
        className="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden grid md:grid-cols-2"
      >
        {/* Left side — Info */}
        <div className="p-8 md:p-10 bg-gradient-to-b from-slate-50 to-white flex flex-col justify-center gap-6">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-3">Teste na prática</h2>
          ...
          {/* Security badges - spaced properly */}
        ...
        {/* Right side — Chat */}
        <div className="flex flex-col h-[500px] bg-white border-l border-slate-100">
          <div className="p-3.5 border-b border-slate-100 flex items-center justify-between bg-white">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-slate-900 text-sm">Agente Visual AI</h4>
          ...
          <div className="flex-1 p-3 overflow-y-auto flex flex-col gap-2.5 bg-slate-50/50">
            {messages.map((msg, i) => (
              <div key={i} className={`max-w-[85%] p-2.5 rounded-2xl text-sm shadow-sm whitespace-pre-line ${
                msg.role === 'user' 
                  ? 'bg-white text-slate-800 rounded-tr-sm border border-slate-200 self-end' 
                  : 'bg-blue-50 border border-blue-100 text-blue-900 rounded-tl-sm self-start'
              }`}>
                {msg.text}
              </div>
            ))}
            {isLoading && (
              <div className="bg-blue-50 border border-blue-100 p-2.5 rounded-2xl rounded-tl-sm max-w-[85%] self-start shadow-sm flex items-center gap-1">
          ...
          <div className="p-3 bg-white border-t border-slate-100 flex gap-2">
            <input 
              ...
              className="flex-1 px-4 py-2 bg-slate-100 border-transparent rounded-full text-sm outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all"
              disabled={isLoading}
            />
            <button 
              onClick={handleSend}
